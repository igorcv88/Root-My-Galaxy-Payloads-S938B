...
## CZG3 reliability and panic-risk instrumentation

The controlled profile now emits records beginning with `RMG_DIAG_V1|`.  Each
record carries a stable run ID, monotonic timestamp, elapsed time, stage,
attempt, classified result, retry-safety decision, cleanup bit, profile, and a
compact state field.  `RUN_START` additionally records `CLOCK_BOOTTIME`, online
CPU count, and load average.  Existing route logs remain the source of the
precise pselect entry/exit duration; the structured attempt record identifies
the run and attempt to which they belong.  Logging occurs at state boundaries,
not in the tight copy/race window.

Immediately before the FOPS race, the payload writes the run/stage/attempt to
`/data/local/tmp/rmg-czg3-last-stage` with `O_DSYNC` when that shell-accessible
location is writable.  Failure to persist is non-fatal.  After a reboot an
Android collector should retain that file and inspect, where permitted by the
build, `/sys/fs/pstore`, `/proc/last_kmsg`, and Samsung dump/ramoops facilities.
The payload does not attempt to read privileged crash records before root.

### Audit result and retry rules

Code inspection confirmed a risky assumption in the legacy slide loop: any
failed child was treated as a clean miss and immediately retried.  A failed
child does **not** prove that the PI futex waiter was dequeued or that no kernel
pointer still refers to the crafted mapping.  CZG3 therefore classifies this as
`RACE_STATE_UNCERTAIN` and aborts the loop.  Only `RACE_NOT_WON` and
`RACE_TIMEOUT_CLEAN`, with affirmative cleanup completion, are `SAFE_RETRY`.
All other classes (`PRECONDITION_FAILED`, `P0_DISCOVERY_FAILED`,
`P0_STATE_INVALID`, `RACE_STATE_UNCERTAIN`, `CLEANUP_FAILED`,
`PRIMITIVE_VALIDATION_FAILED`, and `PRIVILEGE_BOOTSTRAP_FAILED`) are
`UNSAFE_OR_UNKNOWN`.  P0 freshness is reported and the existing fresh-session
requirement is not weakened.

Fake waiter/task lifetime, PI-tree removal after abnormal child termination,
and allocator/slab churn with system age remain hypotheses rather than confirmed
panic causes.  The code establishes the unsafe retry, not the cause of a panic.
Real-device collection should correlate the last durable stage and attempt with
pselect duration, readiness bits, P0 new/reused state, boottime, CPU count,
load, memory pressure from the Android collector, and any post-reboot crash
record.  The uptime observation is supported only if panic-adjacent runs cluster
by boottime after controlling for stage, timing, load, and memory pressure.

### Timing and latency

The policy helper includes bounded timing state and 250-microsecond steps, but
the exploit does not currently apply automatic adjustments: a clean miss does
not expose directional evidence showing whether the trigger was early or late.
Changing delay from outcome alone would be arbitrary.  The helper resets to the
known baseline after ambiguous evidence and is covered by a host test.  The
largest remaining latency opportunity is to preserve deterministic preparation
only after telemetry demonstrates a genuinely clean miss; no preparation or P0
state is currently reused across an uncertain race.
